...
description: 面向用户构建产品时约束 Agent 先验证问题、替代方案和用户摩擦，再按交付级别补齐体验、可靠性、安全、分发与运营要求。用于新产品或 MVP、公开小工具、公司内部产品、商业或高风险产品、功能取舍、竞品与市场调研、核心流程设计和发布准备；按风险分层，避免把所有项目都套进同一套重流程。
license: MIT
metadata:
  author: yhwang303
  version: "1.0.0"
...
不要把个人小工具和正式生产系统套进同一张清单。新产品或首次启用本 Skill 时，先要求用户显式选择或确认 T0–T3，再开始 L1 实现；Prompt 已明确级别时不要重复提问。后续迭代根据用户意图、实际暴露面和失败后果沿用或升级级别，只有级别会显著改变工作量时才重新确认。
